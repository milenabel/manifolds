...
from deeponet_fusion import DeepONetFusion, BranchNet, TrunkNet, FusionMLP
...
def train_fusion_deeponet(config, save_every=1000):
    ...
    deeponet = DeepONetFusion(branch_f_net, branch_n_net, trunk_net, fusion_mlp).to(device)
    ...
    result_path = f"../results/fusion/deeponet_results_torus_N{config['N']}_xi{config['xi']}.json"
    save_results(results, result_path)
    torch.save(deeponet.state_dict(), f"../results/fusion/model_N{config['N']}_xi{config['xi']}.pt")
...
if __name__ == "__main__":
    ...
    for file_path in data_files:
        ...
        print(f"\n--- Training Fusion DeepONet for N={N}, xi={xi} ---")
