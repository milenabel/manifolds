...
# Training Loop
def train_fusion_deeponet(config, save_every=1000):
    ...
    fusion_net = FusionMLP(
        input_dim=256,  # concatenation of branch_f and branch_n outputs
        hidden_dims=[128, 128],
        output_dim=128
    )
    ...
    deeponet = DeepONetFusion2(branch_f_net, branch_n_net, trunk_net, fusion_net).to(device)
    ...
    print(f"\nFinal Generalization Error (Fusion2 DeepONet): {gen_error:.4e}")
    ...
    os.makedirs("../results/fusion2", exist_ok=True)
    result_path = f"../results/fusion2/deeponet_results_torus_N{config['N']}_xi{config['xi']}.json"
    save_results(results, result_path)
    torch.save(deeponet.state_dict(), f"../results/fusion2/model_N{config['N']}_xi{config['xi']}.pt")
...
if __name__ == "__main__":
    ...
    for file_path in data_files:
        ...
        print(f"\n--- Training Fusion2 DeepONet for N={N}, xi={xi} ---")
