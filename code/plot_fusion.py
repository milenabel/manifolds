...
# Settings
xi = 4
fig_save_dir = "../figs/fusion"
model_dir = "../results/fusion"
data_dir = "../data"
hidden_dims = [128, 128]
activation = "relu"
spectral_norm = False
...
for N, epochs, train_errors, test_errors, gen_error in sorted(point_sets):
    ...
    plt.xlabel("Epoch")
    plt.ylabel("Relative L2 Error")
    plt.title(f"Fusion DeepONet (N={N})")
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
...
for model_path in model_files:
    ...
    fusion_net = FusionMLP(2, [32, 32], 1)

    model = DeepONetFusion(branch_f_net, branch_n_net, trunk_net, fusion_net).to(device)
    ...
    plt.suptitle(f"Fusion DeepONet Prediction (N={N}, xi={xi})", fontsize=14)
    plt.tight_layout()
    plt.savefig(f"{fig_save_dir}/torus_comparison_N{N}.png", dpi=300)
    plt.close()

print("All Fusion DeepONet plots and summary saved!")